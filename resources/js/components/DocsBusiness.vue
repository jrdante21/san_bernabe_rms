            ...
            <div class="modal" v-if="modalDocs">
                <div class="content relative">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex-1">
                            <FieldInput fType="select" :fOpts="modalResidentBusiness" v-model="docs.business.value" :fLabel="docs.business.label" :fErrors="docs.business.errors" @onInput="validateData('business')"/>
                        </div>
                        <div class="flex-1">
                        ...
                        <tr v-for="(d, i) in documents.data" :key="i">
                            <td>
                                ...
                                <div class="text-sm italic">
                                    {{ d.business.name }} - {{ d.business.address }}
                                </div>
                            </td>
                            <td>{{ d.doc_num }}</td>
                            <td>{{ d.or_num }}</td>
...
export default {
    ...
    setup () {
        ...
        const docs = reactive({
            business: { value:'', rule:'json', label:'Business' },
    ...
    methods: {
        ...
        editDocuments (data) {
            ...
            for (let k in this.docs) {
                let value = data[k]
                if (k == 'business') {
                    value = JSON.stringify({name:value.name, address:value.address})
                }

                this.docs[k].value = value
            }
        ...
        submitDocs () {
            ...
            let business = JSON.parse(data.business)
    ...
    watch: {
        ...
        modalResidentData (val) {
            if (val.name !== undefined) {
                let business = []
                val.business.forEach(el => {
                    business.push({
                        value: JSON.stringify({name:el.name, address:el.address}),
                        text: `${el.name} - ${el.address}`
                    })
                })

                this.modalResidentBusiness = business
            }
        },
