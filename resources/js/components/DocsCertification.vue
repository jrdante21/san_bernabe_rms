            ...
            <div class="modal" v-if="modalDocs">
                <div class="content relative">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex-1">
                            <FieldInput fType="select" :fOpts="modalResidentAssets" v-model="docs.assets.value" :fLabel="docs.assets.label" :fErrors="docs.assets.errors" @onInput="validateData('assets')"/>
                        </div>
                        <div class="flex-1">
                            <FieldInput fType="textarea" v-model="docs.purpose.value" :fErrors="docs.purpose.errors" :fLabel="docs.purpose.label" @onInput="validateData('purpose')"/>
                        </div>
                        <div class="flex-1">
...
export default {
    ...
    setup () {
        ...
        const docs = reactive({
            assets: { value:'', rule:'json', label:'Assets' },
            purpose: { value:'', rule:'alpha', opts:{hasNum:true}, label:'Purpose' },
            doc_num: { value:'', rule:'alpha', opts:{hasNum:true}, label:'CTC Number' },
    ...
    methods: {
        ...
        editDocuments (data) {
            ...
            for (let k in this.docs) {
                if (k != 'assets') this.docs[k].value = data[k]
            }

            this.docs.purpose.value = data.assets.purpose

            let assets = data.assets.assets
            this.docs.assets.value = JSON.stringify(assets)
        ...
        submitDocs () {
            ...
            let assets = {}
            assets.purpose = data.purpose
            assets.assets = JSON.parse(data.assets)
    ...
    watch: {
        ...
        modalResidentData (val) {
            if (val.name !== undefined) {
                let assets = []
                val.assets.forEach(el => {
                    delete el.resident_id

                    let text = ''
                    switch (el.type) {
                        default:
                        case 'vehicle':
                            text = `Vehicle - Brand: ${el.v_brand}, Model: ${el.v_model}, Plate #: ${el.v_plate},  Color: ${el.v_color}`
                            break;

                        case 'animal':
                            text = `Animal - Type: ${el.a_type}, Sex: ${el.a_sex}, Color: ${el.a_color}, Age: ${el.a_age}, Certificate of Ownership: ${el.a_cert}`
                            break;

                        case 'land':
                            text = `Land - Type: ${el.l_type}, Hectar: ${el.l_hectar}, Location: ${el.l_loc}`
                    }
                ...
                this.modalResidentAssets = assets
            }
        },
