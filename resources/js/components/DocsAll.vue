<template>
    <div>
                        ...
                        <tr v-for="(d, i) in documents.data" :key="i">
                            ...
                            <td>
                                <div class="flex gap-1 justify-end text-sm">
                                    <button v-if="trans.id >= 2" class="button bg-blue-500 text-white" @click="printDocument(d.id)">Print</button>
        ...
        <AlertComponent v-model="assignatoryAlertMsg" message="Assignatory updated successfully." status="success" />
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        submitDocs () {
            ...
            axios.post(`${this.apiServerName}modify_docs`,{data:data, id:this.documentID})
                .then((res) => {
        ...
        printDocument (id) { window.open('/print/document?id='+id, '_blank') },
